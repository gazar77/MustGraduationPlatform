...
- **Student activation OTP (SQL)** — There is no `UserOtp` table. Codes from `POST /api/v1/auth/student/send-code` are stored as the **numeric OTP string** in column **`Code`** on **`StudentLoginOtps`** (existing student user) or **`RegistrationOtps`** (email not registered yet). Query those tables when debugging “no OTP row”; a successful API response (**204**) implies `SaveChanges` ran before email send. The migration that renames `CodeHash` to `Code` clears both tables before shrinking the column to `nvarchar(32)` (old hash values were longer than 32 characters). If `dotnet ef database update` failed mid-way on a host, run `DELETE FROM StudentLoginOtps; DELETE FROM RegistrationOtps;` then apply the migration again.
